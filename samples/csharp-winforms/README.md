# C# WinForms 彩票API调用示例 (JSON)

本目录包含 **C# WinForms 桌面应用** 彩票开奖数据API的完整调用示例，支持 JSON 格式数据解析。
...
- **JSON 格式解析** - 使用 LitJson 库解析 JSON 数据
- **实时开奖查询** - 支持查询最新开奖结果
- **多彩种支持** - 支持 109 个彩票品种
- **批量数据展示** - 显示多期开奖历史
...
1. 使用 Visual Studio 打开 `OCAPIDemo.sln` 解决方案文件
...
```
http://api.byw.bet:868/api?token=您的TOKEN&t=彩种ID&limit=数量&p=json
```
...
http://api.byw.bet:868/api?token=YOUR_TOKEN&t=59&limit=5&p=json

# 北京赛车最新5期数据 (lotteryid: 8)
http://api.byw.bet:868/api?token=YOUR_TOKEN&t=8&limit=5&p=json
```
...
```
csharp-winforms/
├── OCAPIDemo.sln          # Visual Studio 解决方案
...
彩票API接口, 实时开奖数据API, C#彩票SDK, WinForms桌面应用, 彩票数据接口平台, 重庆时时彩API, 广东11选5API, C#桌面程序开发, JSON数据解析, LitJson使用, .NET桌面应用, 彩票开奖结果查询, C# HTTP请求, 博易网API集成
