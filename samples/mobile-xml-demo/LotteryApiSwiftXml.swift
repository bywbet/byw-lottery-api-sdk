/**
 * 彩票API iOS/Swift XML示例
 * 
 * 使用 URLSession 获取数据 + XMLParser 解析 XML
 * 
...
enum LotteryApiError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidResponse:
            return "无效的服务器响应"
        case .xmlParseError(let message):
...
class LotteryXMLParser: NSObject, XMLParserDelegate {
    ...
    private var currentExpect = ""
    private var currentOpencode = ""
    private var currentOpentime = ""
    
    // 解析完成标志
    private var isParsingRow = false
...
class LotteryXmlApiService {
    ...
    private let parser: LotteryXMLParser
    ...
    func getLatest(
        ...
        let urlString = "\(baseURL)?token=\(token)&t=\(lotteryId)&limit=\(limit)&p=xml"
        ...
        let task = session.dataTask(with: request) { [weak self] data, response, error in
            ...
            // 使用 XMLParser 解析
            let results = strongSelf.parser.parse(xmlData: data)
            
            if results.isEmpty {
                // 尝试解析错误信息
                if let xmlString = String(data: data, encoding: .utf8),
                   xmlString.contains("<error>") {
                    completion(.failure(.xmlParseError("API返回错误")))
                } else {
                    completion(.failure(.xmlParseError("解析结果为空")))
                }
            } else {
    ...
    func getByDate(
        ...
        let urlString = "\(baseURL)?token=\(token)&t=\(lotteryId)&date=\(date)&p=xml"
...
#if DEBUG
func demo() {
    let api = LotteryXmlApiService(token: "YOUR_TOKEN_HERE")
